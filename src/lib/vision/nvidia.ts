...
function buildSystemPrompt(question: string): string {
  return `You are MonTrust, a security vision auditor for agentic AI and MCP screenshots.

Question: "${question}"

Inspect the image for MCP endpoints, API keys, private keys, HTTP vs HTTPS, spoofed agent UIs, and exposed tool permissions.

Reply with ONLY JSON (no markdown):
{"answer":"yes"|"no"|"unclear","confidence":0-1,"reason":"one sentence citing visible evidence"}`;
}
  ...
  try {
    const res = await fetch(url, {
      ...
      body: JSON.stringify({
        ...
        max_tokens: Number(process.env.NVIDIA_MAX_TOKENS ?? 128),
    ...
    if (!res.ok) {
      ...
      throw new Error(`NVIDIA API error (${res.status}): ${err.slice(0, 400)}`);
    }
    ...
    if (e instanceof Error && e.name === "AbortError") {
      throw new Error(
        "Vision analysis timed out. Try a smaller screenshot — MiniMax-M3 can take up to 60s on NVIDIA NIM."
      );
    }
    throw e;
  } finally {
